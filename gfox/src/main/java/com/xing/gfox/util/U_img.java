...
public class U_img {
    ...
    public static String imgToStr64(InputStream in) throws IOException {
        ...
        in.close();
        // 对字节数组Base64编码
        return U_base64.encode(data);// 返回Base64编码过的字节数组字符串
    }

    public static String imgToStr64(String imgFile) throws IOException {
        if (U_file.isFileExist(imgFile)) {
    ...
    public static Uri createImagePathUri() {
        ...
        SimpleDateFormat timeFormatter = new SimpleDateFormat(U_time.yyyyMMdd_HHmmss, Locale.getDefault());
