...
public class SConnectionPool {
    ...
    //复用队列
    private final Deque<SHttpConnection> connections = new ArrayDeque<>();

    private boolean cleanupRunning;

    public SConnectionPool() {
        this(1, TimeUnit.MINUTES);
    }

    public SConnectionPool(long keepAliveDuration, TimeUnit timeUnit) {
        //毫秒
    ...
    private final Runnable cleanupRunnable = new Runnable() {
        ...
        public void run() {
            while (true) {
                ...
                if (waitTimes > 0) {
                    synchronized (SConnectionPool.this) {
                        try {
                            //调用某个对象的wait()方法能让当前线程阻塞，
                            // 并且当前线程必须拥有此对象的monitor（即锁）
                            SConnectionPool.this.wait(waitTimes);
    ...
    public SHttpConnection get(String host, int port) {
        Iterator<SHttpConnection> iterator = connections.iterator();
        while (iterator.hasNext()) {
            SHttpConnection connection = iterator.next();
            //查连接是否复用( 同样的host )
    ...
    public void put(SHttpConnection connection) {
        //执行检测清理
        if (!cleanupRunning) {
            cleanupRunning = true;
    ...
    long cleanup(long now) {
        ...
        synchronized (this) {
            for (Iterator<SHttpConnection> i = connections.iterator(); i.hasNext(); ) {
                SHttpConnection connection = i.next();
                //获得闲置时间 多长时间没使用这个了
