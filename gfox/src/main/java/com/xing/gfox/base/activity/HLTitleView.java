...
public class HLTitleView {
    ...
    public HLTitleView(FragmentActivity mActivity, ViewGroup parentView, boolean isAddStatusBar) {
        ...
        titleHeight = (int) mActivity.getResources().getDimension(R.dimen.dp44);
        statusBarHeight = U_screen.getStatusBarHeight(mActivity);
        if (isAddStatusBar) {
            parentView.addView(titleView, ViewGroup.LayoutParams.MATCH_PARENT, titleHeight + U_screen.getStatusBarHeight(mActivity));
        } else {
            parentView.addView(titleView, ViewGroup.LayoutParams.MATCH_PARENT, titleHeight);
        }
    ...
    public void setTitleBar(boolean isShow) {
        if (isShow) {
            U_view.setViewHeight(titleView, titleHeight + statusBarHeight);
        } else {
            U_view.setViewHeight(titleView, titleHeight);
        }
    }
