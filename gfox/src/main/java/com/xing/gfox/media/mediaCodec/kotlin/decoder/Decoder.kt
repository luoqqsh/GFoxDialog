 ...
 * 解码器基类
 */
abstract class Decoder(private val mFilePath: String) : IDecoder {
        ...
        try {
            while (mIsRunning) {
                if (mState != DecodeState.START &&
                    ...
                    //用恢复播放时的系统时间，减去当前正要播放的帧的PTS，得出的值就是mStartTimeForSync
    ...
    private fun init(): Boolean {
        //检查surface，音频直接返回true
        if (!checkSurface()) return false
        //可以支持网络地址，但可能需要调整，待优化。
//        if (!hl_file.isFileExist(mFilePath)) {
//            ViseLog.e(TAG, "文件路径为空")
...
//        }
    ...
    private fun pullBufferFromDecoder(): Int {
        ...
        //调用dequeueOutputBuffer方法查询是否有解码完成的可用数据，其中mBufferInfo用于获取数据帧信息，第二参数是等待时间，这里等待1000ms，填入-1是无限等待。
     ...
     * 检查子类参数
     */
    abstract fun checkSurface(): Boolean
